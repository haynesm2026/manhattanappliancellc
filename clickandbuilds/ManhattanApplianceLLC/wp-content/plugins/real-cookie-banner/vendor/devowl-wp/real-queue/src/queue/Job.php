...
class Job {
    ...
    protected function updateRuns($previousProcess, $force = null) {
        global $wpdb;
        if ($this->process === $previousProcess || $this->process === $this->process_total) {
            if ($force === null) {
                $this->runs += 1;
            } else {
                $this->runs = $force;
    ...
    public function execute() {
        if (!\is_callable($this->callable)) {
            return new \WP_Error(
                'real_queue_job_execute_not_callable',
                __('The passed callable persisted to the job cannot be called (`is_callable`)', REAL_QUEUE_TD)
            );
        }
        ...
        if ($this->locked) {
            return new \WP_Error(
                'real_queue_job_locked',
                __('This job is already running in another thread on your server.', REAL_QUEUE_TD)
            );
        }
        ...
        try {
            ...
            \restore_error_handler();
            return new \WP_Error('real_queue_job_execute_exception', __('Unexpected exception.', REAL_QUEUE_TD), [
                'exception' => [
                    'id' => $this->id,
                    'code' => $e->getCode(),
    ...
    protected function avoidRecurringException() {
        ...
        $failed = $this->core
            ->getQuery()
            ->read(['limit' => self::RECURRING_EXCEPTION_ITEMS, 'type' => 'failure', 'jobType' => $this->type]);
        if (\count($failed) === self::RECURRING_EXCEPTION_ITEMS) {
            $table_name = $this->getTableName();
            $exception = \json_encode([
                'code' => self::RECURRING_EXCEPTION_CODE,
                'message' => null,
                'data' => ['origin' => $this->id]
            ]);
            ...
            $wpdb->query(
                $wpdb->prepare(
                    "UPDATE {$table_name} SET exception = %s, runs = retries + 1 WHERE type = %s AND exception IS NULL AND runs = 0",
                    $exception,
                    $this->type
                )
            );
    ...
    public function omitClientData() {
        if ($this->worker === \DevOwl\RealCookieBanner\Vendor\DevOwl\RealQueue\queue\Job::WORKER_SERVER) {
            $this->data = (object) [];
            $this->callable = null;
        }
    }
