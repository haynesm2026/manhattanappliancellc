...
class ParserState {
    ...
    public function __construct(
        $sText,
        \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Settings $oParserSettings,
        $iLineNo = 1
    ) {
    ...
    public function parseCharacter($bIsForIdentifier) {
        if ($this->peek() === '\\') {
            if (
                $bIsForIdentifier &&
                $this->oParserSettings->bLenientParsing &&
            ...
            if ($this->strlen($sUnicode) < 6) {
                ...
                if (\preg_match('/\\s/isSu', $this->peek())) {
                    if ($this->comes('DevOwl\\RealCookieBanner\\Vendor\\r\\n')) {
                        $this->consume(2);
                    } else {
                        $this->consume(1);
                    }
    ...
    public function consumeWhiteSpace() {
        ...
        do {
            ...
            if ($this->oParserSettings->bLenientParsing) {
                ...
                } catch (\DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Parsing\UnexpectedEOFException $e) {
    ...
    public function consume($mValue = 1) {
        if (\is_string($mValue)) {
            ...
            if ($this->iCurrentPosition + $mValue > $this->iLength) {
                throw new \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Parsing\UnexpectedEOFException(
                    $mValue,
                    $this->peek(5),
                    'count',
                    $this->iLineNo
    ...
    public function consumeComment() {
        ...
        if ($mComment !== \false) {
            ...
            return new \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Comment\Comment(\substr($mComment, 1), $iLineNo);
        }
        return $mComment;
    }
    /**
