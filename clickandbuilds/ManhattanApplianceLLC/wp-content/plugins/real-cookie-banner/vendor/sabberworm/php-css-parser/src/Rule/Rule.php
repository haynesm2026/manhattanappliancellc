...
namespace DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Rule;
...
class Rule implements
    ...
    public static function parse(\DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Parsing\ParserState $oParserState) {
        ...
        $oRule = new \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Rule\Rule(
        ...
        $oValue = \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Value\Value::parseValue(
            $oParserState,
    ...
    public function setValues(array $aSpaceSeparatedValues) {
        ...
        if (\count($aSpaceSeparatedValues) > 1) {
            $oSpaceSeparatedList = new \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Value\RuleValueList(
                ' ',
                $this->iLineNo
            );
        }
        foreach ($aSpaceSeparatedValues as $aCommaSeparatedValues) {
            ...
            if (\count($aCommaSeparatedValues) > 1) {
                $oCommaSeparatedList = new \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Value\RuleValueList(
                    ',',
                    $this->iLineNo
                );
            }
    ...
    public function getValues() {
        if (!$this->mValue instanceof \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Value\RuleValueList) {
            return [[$this->mValue]];
        }
    ...
    public function addValue($mValue, $sType = ' ') {
        ...
        if (
            !$this->mValue instanceof \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Value\RuleValueList ||
            $this->mValue->getListSeparator() !== $sType
        ) {
            $mCurrentValue = $this->mValue;
            $this->mValue = new \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Value\RuleValueList(
                $sType,
                $this->iLineNo
            );
            if ($mCurrentValue) {
    ...
    public function render(\DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\OutputFormat $oOutputFormat) {
        ...
        if ($this->mValue instanceof \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Value\Value) {
