 ...
 */
abstract class AbstractSyncPlugin extends \DevOwl\RealCookieBanner\Vendor\DevOwl\Multilingual\AbstractLanguagePlugin {
    ...
    protected function duplicatePost($post) {
        // Translate name and description
        $args = [
            'post_title' => $this->translateInput($post->post_title)[1],
            'post_content' => $this->translateInput($post->post_content)[1],
    ...
    protected function duplicateMeta($type, $from, $to, $meta) {
        ...
        add_action(
            'added_' . $type . '_meta',
            function ($mid, $object_id, $meta_key, $_meta_value) use (
                $from,
                $to,
                $meta,
                $type,
                ...
                if ($object_id === $from && \in_array($meta_key, $meta, \true)) {
                    ...
                    $this->switchToLanguage($locale, function () use (
                        $type,
                        $from,
                        $to,
                        $meta_key,
    ...
    public function copyPostTaxonomies($from, $to, $taxonomies, $locale) {
        foreach ($taxonomies as $taxonomy) {
            ...
            if (\is_array($originalTerms)) {
                ...
                \DevOwl\RealCookieBanner\Vendor\DevOwl\Multilingual\Utils::withoutFilters(
                    'set_object_terms',
                    function () use ($to, $copyIds, $taxonomy) {
