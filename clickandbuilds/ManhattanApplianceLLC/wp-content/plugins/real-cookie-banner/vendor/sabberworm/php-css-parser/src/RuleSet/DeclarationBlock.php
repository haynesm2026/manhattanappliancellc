...
use DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Parsing\OutputException;
...
use DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Property\KeyframeSelector;
...
use DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Value\Color;
 ...
 */
class DeclarationBlock extends \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\RuleSet\RuleSet {
    /**
    ...
    public static function parse(
        \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Parsing\ParserState $oParserState,
        $oList = null
    ) {
        $aComments = [];
        $oResult = new \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\RuleSet\DeclarationBlock(
        ...
        \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\RuleSet\RuleSet::parseRuleSet($oParserState, $oResult);
        return $oResult;
    }
    /**
    ...
    public function setSelectors($mSelector, $oList = null) {
        ...
        foreach ($this->aSelectors as $iKey => $mSelector) {
            if (!$mSelector instanceof \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Property\Selector) {
                if (
                    $oList === null ||
                    !$oList instanceof \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\CSSList\KeyFrame
                ) {
                    if (!\DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Property\Selector::isValid($mSelector)) {
                        throw new \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Parsing\UnexpectedTokenException(
                            "Selector did not match '" .
                                \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Property\Selector::SELECTOR_VALIDATION_RX .
                                "'.",
                            $mSelector,
                            'custom'
                        );
                    }
                    $this->aSelectors[$iKey] = new \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Property\Selector(
                        $mSelector
                    );
                } else {
                    if (
                        !\DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Property\KeyframeSelector::isValid($mSelector)
                    ) {
                        throw new \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Parsing\UnexpectedTokenException(
                            "Selector did not match '" .
                                \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Property\KeyframeSelector::SELECTOR_VALIDATION_RX .
                                "'.",
                            $mSelector,
                            'custom'
                        );
                    ...
                    $this->aSelectors[
                        $iKey
                    ] = new \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Property\KeyframeSelector($mSelector);
                }
            }
        }
    }
    ...
    public function expandBorderShorthand() {
        ...
        foreach ($aBorderRules as $sBorderRule) {
            ...
            $aValues = [];
            if (!$mRuleValue instanceof \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Value\RuleValueList) {
                $aValues[] = $mRuleValue;
            } else {
                $aValues = $mRuleValue->getListComponents();
            }
            foreach ($aValues as $mValue) {
                if ($mValue instanceof \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Value\Value) {
                    $mNewValue = clone $mValue;
                } else {
                    $mNewValue = $mValue;
                }
                if ($mValue instanceof \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Value\Size) {
                    $sNewRuleName = $sBorderRule . '-width';
                } elseif ($mValue instanceof \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Value\Color) {
                ...
                $oNewRule = new \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Rule\Rule(
                    $sNewRuleName,
                    $oRule->getLineNo(),
                    $oRule->getColNo()
                );
    ...
    public function expandFontShorthand() {
        ...
        foreach ($aValues as $mValue) {
            ...
            if (\in_array($mValue, ['normal', 'inherit'])) {
                ...
                ($mValue instanceof \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Value\Size &&
                    \in_array($mValue->getSize(), \range(100, 900, 100)))
            ) {
                $aFontProperties['font-weight'] = $mValue;
            } elseif (
                $mValue instanceof \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Value\RuleValueList &&
    ...
    public function expandBackgroundShorthand() {
        ...
        $aBgProperties = [
            ...
            'background-position' => [
                new \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Value\Size(0, '%', null, \false, $this->iLineNo),
                new \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Value\Size(0, '%', null, \false, $this->iLineNo)
            ]
        ];
        $mRuleValue = $oRule->getValue();
        ...
        foreach ($aValues as $mValue) {
            ...
            if ($mValue instanceof \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Value\URL) {
    ...
    public function createFontShorthand() {
        ...
        if (isset($aRules['line-height'])) {
            ...
            if ($aLHValues[0] !== 'normal') {
                $val = new \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Value\RuleValueList('/', $this->iLineNo);
        ...
        $oFFValue = new \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Value\RuleValueList(',', $this->iLineNo);
    ...
    public function render(\DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\OutputFormat $oOutputFormat) {
        if (\count($this->aSelectors) === 0) {
            ...
            throw new \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Parsing\OutputException(
