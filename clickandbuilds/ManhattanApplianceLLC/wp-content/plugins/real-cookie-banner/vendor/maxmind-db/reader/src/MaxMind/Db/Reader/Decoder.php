...
class Decoder {
    ...
    public function decode(int $offset): array {
        $ctrlByte = \ord(\DevOwl\RealCookieBanner\Vendor\MaxMind\Db\Reader\Util::read($this->fileStream, $offset, 1));
        ++$offset;
        $type = $ctrlByte >> 5;
        ...
        if ($type === self::_EXTENDED) {
            $nextByte = \ord(
                \DevOwl\RealCookieBanner\Vendor\MaxMind\Db\Reader\Util::read($this->fileStream, $offset, 1)
            );
            $type = $nextByte + 7;
            if ($type < 8) {
                throw new \DevOwl\RealCookieBanner\Vendor\MaxMind\Db\Reader\InvalidDatabaseException(
    ...
    private function decodeByType(int $type, int $offset, int $size): array {
        ...
        $bytes = \DevOwl\RealCookieBanner\Vendor\MaxMind\Db\Reader\Util::read($this->fileStream, $offset, $size);
        switch ($type) {
            case self::_BYTES:
    ...
    private function decodeInt32(string $bytes, int $size): int {
        switch ($size) {
            ...
            case 2:
            case 3:
                $bytes = \str_pad($bytes, 4, "\0", \STR_PAD_LEFT);
                break;
            case 4:
                break;
            default:
    ...
    private function decodePointer(int $ctrlByte, int $offset): array {
        ...
        $buffer = \DevOwl\RealCookieBanner\Vendor\MaxMind\Db\Reader\Util::read(
            $this->fileStream,
            $offset,
            $pointerSize
        );
        ...
        switch ($pointerSize) {
            case 1:
                ...
                break;
            case 2:
                $packed = "\0" . \chr($ctrlByte & 0x7) . $buffer;
                [, $pointer] = \unpack('N', $packed);
    ...
    private function sizeFromCtrlByte(int $ctrlByte, int $offset): array {
        ...
        $bytesToRead = $size - 28;
        $bytes = \DevOwl\RealCookieBanner\Vendor\MaxMind\Db\Reader\Util::read($this->fileStream, $offset, $bytesToRead);
        if ($size === 29) {
            $size = 29 + \ord($bytes);
        } elseif ($size === 30) {
