...
class Queue {
    ...
    public function rest_api_init() {
        ...
        register_rest_route($namespace, '/job/(?P<id>[0-9]+)', [
            'methods' => 'GET',
            'callback' => [$this, 'routeJobGet'],
            'permission_callback' => [$this, 'permission_callback'],
            'args' => ['lock' => ['type' => 'boolean']]
        ]);
        register_rest_route($namespace, '/job/(?P<id>[0-9]+)', [
            'methods' => 'POST',
            'callback' => [$this, 'routeJobExecute'],
            'permission_callback' => [$this, 'permission_callback'],
            'args' => [
                'try' => [
                    'type' => 'string',
        ...
        register_rest_route($namespace, '/job/(?P<id>[0-9]+)/result', [
            'methods' => 'POST',
            'callback' => [$this, 'routeJobResult'],
            'permission_callback' => [$this, 'permission_callback'],
            'args' => [
                'process' => [
                    'type' => 'number',
        ...
        register_rest_route($namespace, '/jobs', [
            'methods' => 'DELETE',
            'callback' => [$this, 'routeJobsDelete'],
            'permission_callback' => [$this, 'permission_callback'],
            'args' => ['type' => ['type' => 'string']]
        ]);
        register_rest_route($namespace, '/jobs/retry', [
            'methods' => 'POST',
            'callback' => [$this, 'routeJobsRetry'],
            'permission_callback' => [$this, 'permission_callback'],
            'args' => ['type' => ['type' => 'string']]
        ]);
        register_rest_route($namespace, '/status', [
            'methods' => 'GET',
            'callback' => [$this, 'routeStatus'],
            'permission_callback' => [$this, 'permission_callback'],
            'args' => ['additionalData' => ['type' => 'string']]
        ]);
    }
    /**
        ...
        $result->passthruClientResult(
            ...
            $errorCode === null
                ...
                : new \WP_Error(
                    $errorCode,
                    $errorMessage,
                    $errorData !== null ? \json_decode($errorData, ARRAY_A) : null
                )
        );
        return new \WP_REST_Response([
            'job' => $result,
            'pauseToAvoidRecurringException' => $result->hasUpdatedJobsToAvoidRecurringException()
        ]);
    }
    /**
     * See API docs.
    ...
    public function routeJobsGet($request) {
        ...
        $data = $query->read([
            'omitClientData' => \true,
            'lockUntil' => self::LOCK_UNTIL_SECONDS,
            'ids' => $ids,
            'after' => $after
        ]);
    ...
    public function routeStatus($request) {
        ...
        return new \WP_REST_Response([
            'currentJobs' => $query->readCurrentJobs(\true),
            'errors' => $executor->buildErrorMessages($remaining),
            'remaining' => $remaining,
            'additionalData' => $additionalData
                ? $this->fetchAdditionalData(\explode(',', $request->get_param('additionalData')))
                : (object) []
        ]);
    }
    /**
    ...
    protected function fetchAdditionalData($additionalData) {
        ...
        foreach ($additionalData as $additionalDataName) {
             ...
             */
            $additionalDataResult = apply_filters(
                'DevOwl/RealQueue/Rest/Status/AdditionalData/' . $additionalDataName,
                null
            );
