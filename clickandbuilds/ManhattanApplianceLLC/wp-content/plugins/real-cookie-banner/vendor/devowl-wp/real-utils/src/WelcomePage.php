...
use DevOwl\RealCookieBanner\Vendor\DevOwl\RealUtils\TransientHandler;
...
class WelcomePage {
    ...
    public function activated_plugin($plugin) {
        ...
        if (!$fromBulkAction && !$isWpCli && $plugin === plugin_basename($this->getInitiator()->getPluginFile())) {
            $alreadyRedirected = \DevOwl\RealCookieBanner\Vendor\DevOwl\RealUtils\TransientHandler::get(
                $this->getInitiator(),
                \DevOwl\RealCookieBanner\Vendor\DevOwl\RealUtils\TransientHandler::TRANSIENT_REDIRECT_AFTER_ACTIVATE,
                \false
            );
            if (!$alreadyRedirected && wp_safe_redirect($this->getWelcomePageLink())) {
                \DevOwl\RealCookieBanner\Vendor\DevOwl\RealUtils\TransientHandler::set(
                    $this->getInitiator(),
                    \DevOwl\RealCookieBanner\Vendor\DevOwl\RealUtils\TransientHandler::TRANSIENT_REDIRECT_AFTER_ACTIVATE,
                    \true
                );
    ...
    <div class="about-wrap-content">
        ...
        $data = [
            ...
            '%subtitle' => \str_replace(
                ...
                \sprintf(
                    '<br /><cite>',
                    \DevOwl\RealCookieBanner\Vendor\DevOwl\RealUtils\Core::getInstance()->getBaseAssetsUrl(
                        'devowl.io-logo.svg'
                    )
                ),
        ...
        return is_admin() &&
            ...
            $_GET['page'] ===
                $this->getInitiator()->getPluginSlug() .
                    \DevOwl\RealCookieBanner\Vendor\DevOwl\RealUtils\WelcomePage::PAGE_SUFFIX;
    }
    /**
     * Create instance.
     *
    ...
    public static function instance($initiator) {
        return new \DevOwl\RealCookieBanner\Vendor\DevOwl\RealUtils\WelcomePage($initiator);
    }
}
