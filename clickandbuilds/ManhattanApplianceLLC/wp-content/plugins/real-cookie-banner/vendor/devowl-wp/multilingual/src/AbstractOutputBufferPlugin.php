 ...
 */
abstract class AbstractOutputBufferPlugin extends
    ...
    protected function wrapHtmlToArray($html, $strip = null) {
        ...
        foreach ($result as &$value) {
            $value = \substr(
                \substr($value, \strlen(self::HTML_TAG_KEEP) + 2),
                0,
                (\strlen(self::HTML_TAG_KEEP) + 3) * -1
            );
     ...
     * @param string $locale
     */
    protected function remapResultToReference(&$content, $result, $locale) {
        foreach ($content as $i => &$untranslated) {
            ...
            if ($previousContent === $translation) {
                $translation = $this->translateStringFromMo($translation, $locale);
            }
