...
class ConfigPage {
    ...
    public function admin_notices() {
        $hasInteractedWithFormOnce = \DevOwl\RealCookieBanner\Core::getInstance()
            ->getRpmInitiator()
            ->getPluginUpdater()
        ...
        if (
            $this->isVisible() ||
            !\DevOwl\RealCookieBanner\Utils::isPreinstalledEnvironment() ||
            $hasInteractedWithFormOnce
        ) {
            return;
        }
        echo \sprintf(
            '<div class="notice notice-warning"><p>%s &bull; <a href="%s">%s</a></p></div>',
            __(
                'A cookie banner for your website is installed. With it, you can integrate e.g. <strong>Google Analytics, Google Maps or YouTube videos</strong> legally compliant. You just have to configure it!',
                RCB_TD
            ),
            $this->getUrl(),
            __('Start now!', RCB_TD)
    ...
    public function admin_menu() {
        $checklistOverdue =
            \DevOwl\RealCookieBanner\view\Checklist::getInstance()->isOverdue(self::CHECKLIST_OVERDUE) &&
            !$this->isVisible();
        /**
        ...
        add_menu_page(
            ...
            __('Cookies', RCB_TD) .
                '&nbsp;' .
                ($checklistOverdue
                    ? ' <span class="awaiting-mod" id="rcb-checklist-overdue" style="margin-right: 5px;">!</span>'
                    : '') .
                '<span class="awaiting-mod" id="rcb-scanner-status" style="display:none;"></span>',
            \DevOwl\RealCookieBanner\Core::MANAGE_MIN_CAPABILITY,
            self::COMPONENT_ID,
            [$this, 'render_component_library'],
            self::getIconAsSvgBase64($this->isVisible() ? '#FFFFFF' : '#858585')
        );
    }
    /**
     ...
     *
     * @param boolean $dismiss If `true`, notice will be dismissed
     */
    public function isProNoticeVisible($dismiss = null) {
        if ($this->isPro() || (!$this->isVisible() && $dismiss === null)) {
            return \false;
        }
        $next = get_option(self::OPTION_NAME_NEXT_PRO_NOTICE, 0);
        if ($next === 0 || $dismiss === \true) {
            update_option(self::OPTION_NAME_NEXT_PRO_NOTICE, \time() + self::NEXT_PRO_NOTICE, \true);
            return \false;
        }
        return \time() > $next;
    }
