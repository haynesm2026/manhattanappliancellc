...
class Color extends \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Value\CSSFunction {
    ...
    public static function parse(\DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Parsing\ParserState $oParserState) {
        ...
        if ($oParserState->comes('#')) {
            ...
            if ($oParserState->strlen($sValue) === 8) {
                $aColor = [
                    'r' => new \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Value\Size(
                        \intval($sValue[0] . $sValue[1], 16),
                        null,
                        \true,
                        $oParserState->currentLine()
                    ),
                    'g' => new \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Value\Size(
                        \intval($sValue[2] . $sValue[3], 16),
                        null,
                        \true,
                        $oParserState->currentLine()
                    ),
                    'b' => new \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Value\Size(
                        \intval($sValue[4] . $sValue[5], 16),
                        null,
                        \true,
                        $oParserState->currentLine()
                    ),
                    'a' => new \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Value\Size(
            ...
            for ($i = 0; $i < $iLength; ++$i) {
                ...
                if ($oParserState->comes('var')) {
                    $aColor[
                        $sColorMode[$i]
                    ] = \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Value\CSSFunction::parseIdentifierOrFunction(
                        $oParserState
                    );
                    $bContainsVarOrCalc = \true;
                } elseif ($oParserState->comes('calc')) {
                    $aColor[$sColorMode[$i]] = \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Value\CalcFunction::parse(
                        $oParserState
                    );
                    $bContainsVarOrCalc = \true;
                } else {
                    $aColor[$sColorMode[$i]] = \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Value\Size::parse(
                        $oParserState,
                        \true
                    );
                }
    ...
    public function render(\DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\OutputFormat $oOutputFormat) {
        ...
        if ($oOutputFormat->getRGBHashNotation() && \implode('', \array_keys($this->aComponents)) === 'rgb') {
            ...
            return '#' .
                ($sResult[0] == $sResult[1] && $sResult[2] == $sResult[3] && $sResult[4] == $sResult[5]
                    ? "{$sResult[0]}{$sResult[2]}{$sResult[4]}"
                    : $sResult);
        }
