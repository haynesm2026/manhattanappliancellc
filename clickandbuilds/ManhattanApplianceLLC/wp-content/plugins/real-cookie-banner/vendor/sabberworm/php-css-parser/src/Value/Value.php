...
abstract class Value implements \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Renderable {
    /**
     * @var int
     */
    protected $iLineNo;
    ...
    public static function parseValue(
        ...
        foreach ($aListDelimiters as $sDelimiter) {
            ...
            while (($iStartPosition = \array_search($sDelimiter, $aStack, \true)) !== \false) {
                ...
                $oList = new \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Value\RuleValueList(
                    $sDelimiter,
     ...
     * @throws UnexpectedTokenException
     */
    public static function parseIdentifierOrFunction(
        \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Parsing\ParserState $oParserState,
        $bIgnoreCase = \false
    ) {
        ...
        if ($oParserState->comes('(')) {
            ...
            $aArguments = \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Value\Value::parseValue($oParserState, [
                '=',
                ' ',
                ','
            ]);
            $sResult = new \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Value\CSSFunction(
                $sResult,
                $aArguments,
                ',',
                $oParserState->currentLine()
    ...
    public static function parsePrimitiveValue(
        ...
        if (
            ...
            ($oParserState->comes('-.') && \is_numeric($oParserState->peek(1, 2))) ||
            (($oParserState->comes('-') || $oParserState->comes('.')) && \is_numeric($oParserState->peek(1, 1)))
        ) {
            $oValue = \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Value\Size::parse($oParserState);
        } elseif (
            $oParserState->comes('#') ||
            $oParserState->comes('rgb', \true) ||
            $oParserState->comes('hsl', \true)
        ) {
            $oValue = \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Value\Color::parse($oParserState);
        } elseif ($oParserState->comes('url', \true)) {
            $oValue = \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Value\URL::parse($oParserState);
        } elseif (
            $oParserState->comes('calc', \true) ||
            $oParserState->comes('-webkit-calc', \true) ||
            $oParserState->comes('-moz-calc', \true)
        ) {
            $oValue = \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Value\CalcFunction::parse($oParserState);
        } elseif ($oParserState->comes("'") || $oParserState->comes('"')) {
            $oValue = \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Value\CSSString::parse($oParserState);
            ...
            $oValue = \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Value\LineName::parse($oParserState);
     ...
     * @throws UnexpectedTokenException
     */
    private static function parseMicrosoftFilter(
        \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Parsing\ParserState $oParserState
    ) {
        $sFunction = $oParserState->consumeUntil('(', \false, \true);
        $aArguments = \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Value\Value::parseValue($oParserState, [',', '=']);
        return new \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Value\CSSFunction(
            $sFunction,
            $aArguments,
            ',',
            $oParserState->currentLine()
     ...
     * @throws UnexpectedTokenException
     */
    private static function parseUnicodeRangeValue(
        \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Parsing\ParserState $oParserState
    ) {
