...
class Assets {
    ...
    public function overrideLocalizeScript($context) {
        $ratingHandler = \DevOwl\RealCookieBanner\Vendor\DevOwl\RealUtils\Core::getInstance()->getRatingHandler();
        // Get names of plugins
        $names = [];
        foreach (\DevOwl\RealCookieBanner\Vendor\DevOwl\RealUtils\Core::getInstance()->getInitiators() as $initiator) {
        ...
        return [
            ...
            'cross' => \DevOwl\RealCookieBanner\Vendor\DevOwl\RealUtils\Core::getInstance()
    ...
    protected function enqueueCrossSelling() {
        if (
            \count(
                \DevOwl\RealCookieBanner\Vendor\DevOwl\RealUtils\Core::getInstance()
                    ->getCrossSellingHandler()
                    ->getAvailable()
            ) > 0
        ) {
    ...
    public static function instance() {
        return new \DevOwl\RealCookieBanner\Vendor\DevOwl\RealUtils\Assets();
    }
}
