...
use DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Value\ValueList;
/**
 ...
 */
abstract class CSSBlockList extends \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\CSSList\CSSList {
    /**
     * @param int $iLineNo
     */
    ...
    protected function allDeclarationBlocks(array &$aResult) {
        foreach ($this->aContents as $mContent) {
            if ($mContent instanceof \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\RuleSet\DeclarationBlock) {
                $aResult[] = $mContent;
            } elseif ($mContent instanceof \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\CSSList\CSSBlockList) {
    ...
    protected function allRuleSets(array &$aResult) {
        foreach ($this->aContents as $mContent) {
            if ($mContent instanceof \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\RuleSet\RuleSet) {
                $aResult[] = $mContent;
            } elseif ($mContent instanceof \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\CSSList\CSSBlockList) {
    ...
    protected function allValues(
        ...
        if ($oElement instanceof \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\CSSList\CSSBlockList) {
            foreach ($oElement->getContents() as $oContent) {
                $this->allValues($oContent, $aResult, $sSearchString, $bSearchInFunctionArguments);
            }
        } elseif ($oElement instanceof \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\RuleSet\RuleSet) {
            foreach ($oElement->getRules($sSearchString) as $oRule) {
                $this->allValues($oRule, $aResult, $sSearchString, $bSearchInFunctionArguments);
            }
        } elseif ($oElement instanceof \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Rule\Rule) {
            $this->allValues($oElement->getValue(), $aResult, $sSearchString, $bSearchInFunctionArguments);
        } elseif ($oElement instanceof \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Value\ValueList) {
            if (
                $bSearchInFunctionArguments ||
                !$oElement instanceof \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Value\CSSFunction
            ) {
